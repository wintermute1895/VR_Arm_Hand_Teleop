...
import numpy as np
from utils_1 import rotation_matrix, translation_matrix
...
class HandForwardKinematics:
    ...
    def calculate_fk_all_keypoints(self, joint_angles_dict):
        ...
        for finger in self.config.FINGER_NAMES:
            ...
            # 处理mimic关节
            if 'mimic' in self.config.JOINT_INFO[finger]:
                for mimic_joint, rule in self.config.JOINT_INFO[finger]['mimic'].items():
                    source_joint_name = rule['joint']
                    source_finger = \
                    [f for f, info in self.config.JOINT_INFO.items() if source_joint_name in info['names']][0]
                    source_joint_idx = self.config.JOINT_INFO[source_finger]['names'].index(source_joint_name)
                    mimic_joint_idx = self.config.JOINT_INFO[finger]['names'].index(mimic_joint)
