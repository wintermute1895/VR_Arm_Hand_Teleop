# config_l10.py
# (根据 linkerhand_l10_7_left.urdf 和 linkerhand_l10_7_right.urdf 精确生成)
...
# --- 右手参数 (手动镜像左手参数，硬编码) ---
RIGHT_FINGER_BASE_POSITIONS = {
    'thumb':  np.array([0.013419, -0.012551, 0.060602]), # X坐标取反
    'index':  np.array([0.0021643, -0.026654, 0.13253]),  # X坐标取反
    'middle': np.array([0.0021316, -0.0076542, 0.15281]), # X坐标取反
    'ring':   np.array([0.0021643, 0.011346, 0.13253]),   # X坐标取反
    'pinky':  np.array([0.00012074, 0.030346, 0.12755])   # X坐标取反
}

RIGHT_JOINT_INFO = {
    'thumb': {
        'names': ['thumb_cmc_roll', 'thumb_cmc_yaw', 'thumb_cmc_pitch', 'thumb_mcp', 'thumb_ip'],
        'axes': [np.array(x) for x in [[0.99996, 0, 0.0087265], [0.008517, -0.21782, 0.97595], [0, 1, 0], [0, 1, 0], [0, 1, 0]]], # X轴和Z轴根据URDF进行反转
        'limits': [(0, 1.1339), (0, 1.9189), (0, 0.5149), (0, 0.7152), (0, 0.7763)],
        'offsets': [
            np.array([-0.035797, 0.00065879, 0.00045944]), # X分量取反
            np.array([-0.0046051, -0.014383, -0.0051478]),# X分量取反
            np.array([-0.0061722, 0, 0.047968]),         # X分量取反
            np.array([0.00017064, 0, 0.038665]),        # X分量取反
            np.array([0, 0, 0.025])
        ],
        'mimic': {
            'thumb_mcp': {'joint': 'thumb_cmc_pitch', 'multiplier': 1.3898, 'offset': 0.0},
    ...
    'index': {
        'names': ['index_mcp_roll', 'index_mcp_pitch', 'index_pip', 'index_dip'],
        'axes': [np.array(x) for x in [[-0.99996, 0, -0.0087265], [0, 1, 0], [0, 1, 0], [0, 1, 0]]], # X轴反转
        'limits': [(0, 0.2181), (0, 1.3607), (0, 1.8317), (0, 0.628)],
        'offsets': [
            np.array([-0.0020763, 0, 0.015294]), # X分量取反
            np.array([0.0013807, 0, 0.035624]),  # X分量取反
            np.array([0.0054686, 0, 0.025665]),  # X分量取反
            np.array([0, 0, 0.020])
        ],
        'mimic': {
            'index_pip': {'joint': 'index_mcp_pitch', 'multiplier': 1.3462, 'offset': 0.0},
    ...
    'middle': {
        ...
        'limits': [(0, 1.3607), (0, 1.8317), (0, 1.628)],
        'offsets': [
            np.array([0.001397, 0, 0.035623]), # X分量取反
            np.array([0.0055098, 0, 0.025656]), # X分量取反
            np.array([0, 0, 0.020])
        ],
        'mimic': {
            'middle_pip': {'joint': 'middle_mcp_pitch', 'multiplier': 1.3462, 'offset': 0.0},
...
# --- [新增] 左右手参数的总入口 ---
HAND_BASE_POSITIONS_MAP = {
    'left': LEFT_FINGER_BASE_POSITIONS,
    'right': RIGHT_FINGER_BASE_POSITIONS
}

JOINT_INFO_MAP = {
    'left': LEFT_JOINT_INFO,
    'right': RIGHT_JOINT_INFO
}


# --- 动态获取参数的函数 (在其他模块中调用) ---
def get_finger_base_positions(hand_type):
    return HAND_BASE_POSITIONS_MAP.get(hand_type, RIGHT_FINGER_BASE_POSITIONS) # 默认左手

def get_joint_info(hand_type):
    return JOINT_INFO_MAP.get(hand_type, RIGHT_JOINT_INFO) # 默认左手
