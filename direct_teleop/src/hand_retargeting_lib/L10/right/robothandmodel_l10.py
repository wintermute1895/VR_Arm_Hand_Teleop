#### **2. `robothandmodel_l10.py` (核心：接收 `hand_type` 并应用镜像变换)**

# robothandmodel_l10.py
# (最终修正版：支持左右手，并应用PICO输入镜像变换)

import numpy as np
from scipy.spatial.transform import Rotation
import copy # 用于深度复制字典

from src.hand_retargeting_lib.L10.fk_l10 import HandForwardKinematics_L10
from src.hand_retargeting_lib.L10.utils_l10 import sigmoid, translation_matrix
...
class RobotHandModel_L10:
    ...
    def update_human_hand_state(self, new_w_dict):
        # 核心：对 PICO 人手关键点进行镜像变换，以匹配机械手的右手运动学
        if self.hand_type == 'right':
            mirrored_w_dict = {}
            for key, points in new_w_dict.items():
                if isinstance(points, np.ndarray) and points.size > 0:
                    mirrored_points = np.copy(points)
                    mirrored_points[:, 0] *= -1 # X坐标取反
                    mirrored_w_dict[key] = mirrored_points
                else:
                    mirrored_w_dict[key] = points
            self.w = mirrored_w_dict
        else: # 如果是左手，则直接使用
            self.w = new_w_dict
    ...
    def calibrate(self, w_star_dict):
        ...
        # !! 核心修正 2：确保调用统一后的函数名 !!
        # 此处 w_star_dict 已经在 update_human_hand_state 中被镜像处理过了
        # 所以这里的 w_wrist, w_index_mcp, w_pinky_mcp 已经是镜像后的数据
    ...
    def _compute_v(self):
        ...
        for finger in self.config.FINGER_NAMES: # 使用config.FINGER_NAMES以计算所有模型
